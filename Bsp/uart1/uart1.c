...
uint8_t dma_rx_buffer[DMA_RX_BUF_SIZE]; // 硬件 DMA 专用数组
...
volatile uint16_t tx_readIndex = 0;
volatile uint16_t tx_writeIndex = 0;
volatile uint8_t tx_busy = 0;
volatile uint16_t current_tx_len = 0;

//// RTOS 发送保护资源
//static SemaphoreHandle_t tx_mutex;      // 防止多任务争抢发送
//static SemaphoreHandle_t tx_space_sem;  // 缓冲满时，阻塞任务等待 DMA 发送腾出空间

void Uart_TxProcess(void); // 提前声明
...
//    tx_space_sem = xSemaphoreCreateBinary();
    ...
    HAL_UARTEx_ReceiveToIdle_DMA(&huart1, dma_rx_buffer, DMA_RX_BUF_SIZE);
 ...
 * @brief 计算待发送的数据长度
 */
uint16_t Uart_TxGetLength(void) {
    return (tx_writeIndex + TX_BUFFER_SIZE - tx_readIndex) % TX_BUFFER_SIZE;
}

/**
 * @brief 计算发送缓冲区剩余空间
 */
uint16_t Uart_TxGetRemain(void) {
    return TX_BUFFER_SIZE - 1 - Uart_TxGetLength();
}
...
uint16_t Uart_Send_RTOS(uint8_t *data, uint16_t length) {
    ...
    {
        uint16_t remain = Uart_TxGetRemain();
        
        // 如果软件缓冲区满了怎么办？
        if (remain == 0) {
            Uart_TxProcess(); // 确保 DMA 正在工作
        ...
        uint16_t tail_len = TX_BUFFER_SIZE - tx_writeIndex;
        
        if (to_write <= tail_len) {
            memcpy(tx_buffer + tx_writeIndex, data + written, to_write);
        } else {
            memcpy(tx_buffer + tx_writeIndex, data + written, tail_len);
            memcpy(tx_buffer, data + written + tail_len, to_write - tail_len);
        }
        
        tx_writeIndex = (tx_writeIndex + to_write) % TX_BUFFER_SIZE;
        written += to_write;
    }
...
void Uart_TxProcess(void) {
    ...
    // 5. 计算并启动发送
    uint16_t len = Uart_TxGetLength();
    uint16_t tail_len = TX_BUFFER_SIZE - tx_readIndex;

    if (len <= tail_len) {
        current_tx_len = len;      
    } else {
        current_tx_len = tail_len; 
    }

    if (HAL_UART_Transmit_DMA(&huart1, &tx_buffer[tx_readIndex], current_tx_len) != HAL_OK) {     
        tx_busy = 0; 
    }
